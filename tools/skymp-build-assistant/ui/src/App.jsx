...
export function App() {
  ...
  async function handleSyncServerLoadOrder() {
    try {
      await syncServerLoadOrder();
      setNotice({
        type: 'success',
        title: 'Server load order synced',
        message: server?.running
          ? 'server-settings.json now matches Plugins.txt. Restart the server to apply the new plugin list.'
          : 'server-settings.json now matches the enabled Plugins.txt load order.',
      });
      await loadDoctor();
    } catch (error) {
      setNotice({
        type: 'error',
        title: 'Could not sync server load order',
        message: error.message || 'Updating server-settings.json failed.',
      });
      await refreshMods({ silent: true });
      await loadDoctor();
    }
    ...
    <div className="app-shell">
      <header className="app-header">
        ...
        <div className="header-actions">
          ...
          <div className="header-actions__status">
            <span className="header-actions__last-updated">
              <span className="status-dot status-dot--ok" />
              {formatLastUpdatedLabel(lastUpdatedAt)}
            </span>
            <button
              type="button"
              className={`header-status-chip header-status-chip--${server?.running ? 'ok' : 'bad'}`}
              onClick={() => setActiveTab('server')}
            >
              <span className={`status-dot status-dot--${server?.running ? 'ok' : 'bad'}`} />
              <span>{server?.running ? 'Server running' : 'Server stopped'}</span>
            </button>
          </div>
        </div>
      </header>
            ...
            <section className="panel panel--page">
              <div className="panel__header panel__header--wrap">
                <div>
                  ...
                  <br />
                  <h3>Notice about Vortex support:</h3>
                  <p>Vortex is not supported yet. The assistant will not be able to sync mods from Vortex. Mixing these tools will result in unexpected behavior.</p>
                </div>
                <div className="button-row">
                  <button
                    type="button"
                    className="ghost-button"
                    disabled={modsRefreshing || modsReordering || syncingServerLoadOrder}
                    onClick={() => void refreshMods({ silent: true })}
                  >
                    <SymbolIcon name="refresh" className="button-icon" />
                    <span>
                      {modsRefreshing || modsReordering || syncingServerLoadOrder
                        ? 'Refreshing...'
                        : 'Refresh mods'}
                    </span>
                  </button>
              ...
              <div className="mod-notices-grid summary-grid">
              ...
              <div className="inline-summary">
                <strong>Server load order</strong>
                <p>
                  Comparing enabled Plugins.txt entries against{' '}
                  <code>{formatPath(modsSnapshot.serverLoadOrder?.path)}</code>.
                </p>
                {modsSnapshot.serverLoadOrder?.comparisonReady ? (
                  <p>
                    {modsSnapshot.serverLoadOrder.matchesPluginsTxt
                      ? 'Server settings already match the current Plugins.txt order.'
                      : 'Server settings are out of sync with the current Plugins.txt order.'}
                  </p>
                ) : (
                  <p>
                    {modsSnapshot.serverLoadOrder?.readError ||
                      'Build the server first so the assistant can compare against server-settings.json.'}
                  </p>
                )}
                {modsSnapshot.serverLoadOrder?.missingFromServer?.length ? (
                  <p>
                    Missing from server:{' '}
                    <code>{modsSnapshot.serverLoadOrder.missingFromServer.slice(0, 8).join(', ')}</code>
                  </p>
                ) : null}
                {modsSnapshot.serverLoadOrder?.extraOnServer?.length ? (
                  <p>
                    Extra on server:{' '}
                    <code>{modsSnapshot.serverLoadOrder.extraOnServer.slice(0, 8).join(', ')}</code>
                  </p>
                ) : null}
                {modsSnapshot.serverLoadOrder?.orderMismatch ? (
                  <p>The shared plugin list is ordered differently in server-settings.json.</p>
                ) : null}
                {server?.running ? (
                  <p>Server setting changes only take effect after restarting the server.</p>
                ) : null}
                <div className="button-row">
                  <button
                    type="button"
                    ...
                    disabled={
                      ...
                      modsRefreshing ||
                      modsReordering ||
                      !modsSnapshot.serverLoadOrder?.syncable
                    }
                    onClick={() => void handleSyncServerLoadOrder()}
                  >
                    <SymbolIcon name="refresh" className="button-icon" />
                    <span>{syncingServerLoadOrder ? 'Syncing...' : 'Sync server load order'}</span>
                  </button>
                  <button
                    type="button"
                    className="ghost-button"
