...
import { discoverAddonManifests, mergeAddonManifestData } from './addon-manifests.js';
...
export function loadWorkspace({ repoRoot, manifestPath, configPath }) {
  ...
  const addons = discoverAddonManifests(repoRoot, ctx);
  const mergedManifest = mergeAddonManifestData(expanded, addons);
  return { manifestDoc: mergedManifest, ctx, config };
}
...
export function watchPathsForManifest(manifestDoc, ctx) {
  /** @type {string[]} */
  const paths = [];
  const seen = new Set();
  const pushUnique = (candidatePath) => {
    if (!candidatePath) {
      return;
    }
  ...
  const getWatchPathForSource = (sourcePath, kind) => {
    if (!sourcePath) {
      return '';
    }

    const directPath = kind === 'directory' ? sourcePath : path.dirname(sourcePath);
    if (fs.existsSync(directPath)) {
      return directPath;
    }

    const parentPath = path.dirname(directPath);
    if (parentPath && parentPath !== directPath && fs.existsSync(parentPath)) {
      return parentPath;
    }
  ...
  for (const artifact of manifestDoc.artifacts ?? []) {
    ...
    for (const source of artifact.sources ?? []) {
      pushUnique(getWatchPathForSource(source?.path, kind));
    }
  }

  if (paths.length === 0) {
    console.warn(
      '[skymp-dev watch] Nothing to watch yet - build once so manifest artifact outputs exist.',
    );
  }
  return paths;
}
