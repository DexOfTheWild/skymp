...
export function runDoctor(repoRoot, ctx) {
  ...
  if (ctx.skyrimRoot && fs.existsSync(ctx.skyrimExePath)) {
    ...
    if (modsSnapshot.serverLoadOrder.path && modsSnapshot.serverLoadOrder.readable === false) {
      issues.push({
        level: 'warn',
        code: 'server_load_order_unreadable',
        message:
          `Server load order could not be read at ${modsSnapshot.serverLoadOrder.path}: ${modsSnapshot.serverLoadOrder.readError}`,
      });
    } else if (
      modsSnapshot.serverLoadOrder.comparisonReady &&
      modsSnapshot.serverLoadOrder.matchesPluginsTxt === false
    ) {
      const details = [];
      if (modsSnapshot.serverLoadOrder.missingFromServer.length > 0) {
        details.push(
          `missing from server: ${modsSnapshot.serverLoadOrder.missingFromServer.slice(0, 5).join(', ')}`,
        );
      }
      if (modsSnapshot.serverLoadOrder.extraOnServer.length > 0) {
        details.push(
          `extra on server: ${modsSnapshot.serverLoadOrder.extraOnServer.slice(0, 5).join(', ')}`,
        );
      }
      if (modsSnapshot.serverLoadOrder.orderMismatch) {
        details.push('shared plugins are ordered differently');
      }
      issues.push({
        level: 'warn',
        code: 'server_load_order_mismatch',
        message:
          `Server load order in ${modsSnapshot.serverLoadOrder.path} does not match Plugins.txt` +
