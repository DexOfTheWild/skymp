...
import {
  getDesiredServerLoadOrder,
  readServerLoadOrderDocument,
  writeServerLoadOrderDocument,
} from './server-load-order.js';
...
function getEnabledPluginNames(pluginsDoc) {
  return (pluginsDoc.orderedPluginNames ?? []).filter((pluginName) => {
    return pluginsDoc.pluginStateByName.get(pluginKey(pluginName))?.enabled === true;
  });
}
...
function buildServerLoadOrderStatus(ctx, pluginsDoc) {
  const pluginsReadable = pluginsDoc.readable !== false;
  const serverDoc = readServerLoadOrderDocument(ctx);
  const clientPluginNames = pluginsReadable ? getEnabledPluginNames(pluginsDoc) : [];
  const serverPluginNames = serverDoc.customEntries.map((entry) => entry.pluginName);
  const clientPluginKeys = new Set(clientPluginNames.map((pluginName) => pluginKey(pluginName)));
  const serverPluginKeys = new Set(serverPluginNames.map((pluginName) => pluginKey(pluginName)));
  const missingFromServer = clientPluginNames.filter(
    (pluginName) => !serverPluginKeys.has(pluginKey(pluginName)),
  );
  const extraOnServer = serverPluginNames.filter(
    (pluginName) => !clientPluginKeys.has(pluginKey(pluginName)),
  );
  const orderMismatch =
    missingFromServer.length === 0 &&
    extraOnServer.length === 0 &&
    clientPluginNames.length === serverPluginNames.length &&
    clientPluginNames.some(
      (pluginName, index) => pluginKey(pluginName) !== pluginKey(serverPluginNames[index]),
    );
  const comparisonReady = pluginsReadable && serverDoc.readable === true;

  return {
    path: serverDoc.path,
    exists: serverDoc.exists === true,
    readable: serverDoc.readable === true,
    readError: serverDoc.readError ?? '',
    dataDir: String(serverDoc.data?.dataDir ?? ''),
    trackedPlugins: serverPluginNames.length,
    vanillaPlugins: serverDoc.vanillaEntries.length,
    comparisonReady,
    matchesPluginsTxt:
      comparisonReady &&
      missingFromServer.length === 0 &&
      ...
      orderMismatch === false,
    syncable:
      comparisonReady && Boolean(ctx?.skyrimDataDir) && fs.existsSync(String(ctx?.skyrimDataDir || '')),
    clientPluginNames,
    serverPluginNames,
    missingFromServer,
    extraOnServer,
...
export function getModsSnapshot(runtime, options = {}) {
  ...
  return {
    ...
    serverLoadOrder: buildServerLoadOrderStatus(runtime?.ctx ?? {}, pluginsDoc),
    mods: mergedMods,
...
export function syncServerLoadOrder(runtime, options = {}) {
  const runtimeStatus = options.runtimeStatus ?? null;
  const snapshot = getModsSnapshot(runtime, { runtimeStatus });

  if (snapshot.pluginsTxt.readable === false) {
    throw createModsError(
      snapshot.pluginsTxt.readError || 'Plugins.txt could not be read.',
      'MOD_PLUGINS_TXT_UNREADABLE',
      { snapshot },
    );
  }

  if (snapshot.serverLoadOrder.syncable !== true) {
    throw createModsError(
      snapshot.serverLoadOrder.readError ||
        'server-settings.json is not available yet. Build the server first.',
      'MOD_SERVER_LOAD_ORDER_UNAVAILABLE',
      { snapshot },
    );
  }

  const enabledPluginNames = snapshot.serverLoadOrder.clientPluginNames ?? [];
  const skyrimDataDir = String(runtime?.ctx?.skyrimDataDir || '');
  const missingPluginNames = enabledPluginNames.filter((pluginName) => {
    return !fs.existsSync(path.join(skyrimDataDir, pluginName));
  });
  if (missingPluginNames.length > 0) {
    throw createModsError(
      `Enabled plugins are missing from Skyrim Data: ${missingPluginNames.slice(0, 5).join(', ')}`,
      'MOD_SERVER_LOAD_ORDER_PLUGIN_MISSING',
      {
        snapshot,
        missingPluginNames,
      },
  ...
  const serverDoc = readServerLoadOrderDocument(runtime?.ctx ?? {});
  if (serverDoc.readable !== true) {
    throw createModsError(
      serverDoc.readError || 'server-settings.json could not be read.',
      'MOD_SERVER_LOAD_ORDER_UNAVAILABLE',
      { snapshot },
    );
  }

  const nextLoadOrder = getDesiredServerLoadOrder(runtime?.ctx ?? {}, serverDoc, enabledPluginNames);
  writeServerLoadOrderDocument(serverDoc, nextLoadOrder);
