...
import { getAddonBuildExpectedOutputs } from './addon-manifests.js';
...
function buildState(runtime) {
  ...
  return {
    ...
    cmakeConfigured,
    currentPackageRoot,
    discoveredAddons: runtime.manifestDoc?.__addons ?? [],
    frontDir,
    gamemodeDir,
    functionsLibDir,
  };
...
function sanitizeAddonKey(value) {
  return String(value).replace(/[^A-Za-z0-9_-]+/g, '-');
}

function addonProfileId(addon) {
  return `addon-${addon.addonId}`;
}

function getDiscoveredAddons(state) {
  return state.discoveredAddons ?? [];
}

function getBuildableAddons(state) {
  return getDiscoveredAddons(state).filter((addon) => addon.build);
}

function getCmakeGeneratorName(state) {
  return String(state.cmakeCache?.CMAKE_GENERATOR ?? '');
}

function isVisualStudioGenerator(state) {
  return getCmakeGeneratorName(state).startsWith('Visual Studio');
}

function getVisualStudioBuildPlatform(state) {
  return String(state.cmakeCache?.CMAKE_GENERATOR_PLATFORM ?? '').trim() || 'x64';
}

function getSolutionPath(state) {
  return path.join(state.buildDir, 'skymp.sln');
}

function getVisualStudioProjectPath(state, projectName) {
  const directCandidate = path.join(state.buildDir, projectName, `${projectName}.vcxproj`);
  if (fs.existsSync(directCandidate)) {
    return directCandidate;
  }

  const rootLevelCandidate = path.join(state.buildDir, `${projectName}.vcxproj`);
  if (fs.existsSync(rootLevelCandidate)) {
    return rootLevelCandidate;
  }

  const matches = fs
    .readdirSync(state.buildDir, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => path.join(state.buildDir, entry.name, `${projectName}.vcxproj`))
  ...
  const solutionPath = getSolutionPath(state);
  if (!fs.existsSync(solutionPath)) {
    return '';
  }

  const solutionText = fs.readFileSync(solutionPath, 'utf8');
  const normalizedProjectName = String(projectName).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const matcher = new RegExp(
    `Project\\([^\\n]+\\)\\s*=\\s*"${normalizedProjectName}"\\s*,\\s*"([^"]+\\.vcxproj)"`,
    'm',
  );
  const match = solutionText.match(matcher);
  if (!match?.[1]) {
    return '';
  }

  return path.join(state.buildDir, match[1].replace(/\\/g, path.sep));
}

function hasVisualStudioProject(state, projectName) {
  if (!isVisualStudioGenerator(state)) {
    return false;
  }

  return Boolean(getVisualStudioProjectPath(state, projectName));
}

function getVisualStudioMsBuildPath(state) {
  if (!isVisualStudioGenerator(state)) {
    return '';
  }

  const generatorInstance = String(state.cmakeCache?.CMAKE_GENERATOR_INSTANCE ?? '').trim();
  if (generatorInstance) {
    const explicitPath = path.join(generatorInstance, 'MSBuild', 'Current', 'Bin', 'MSBuild.exe');
    if (fs.existsSync(explicitPath)) {
      return explicitPath;
    }
  }

  return commandExists('MSBuild') ? 'MSBuild' : '';
}

function hasVisualStudioMsBuild(state) {
  return Boolean(getVisualStudioMsBuildPath(state));
}

function getAddonsByBuildKind(state, buildKind) {
  return getBuildableAddons(state).filter((addon) => addon.build?.kind === buildKind);
}

function getCmakeTargetAddons(state) {
  return getAddonsByBuildKind(state, 'cmake-target');
}

function getCommandAddons(state) {
  return getAddonsByBuildKind(state, 'command');
}

function isAddonIncludedInBuildAll(addon) {
  return addon.build?.includedInBuildAll !== false;
}

function collectAddonExpectedOutputs(addons) {
  const outputs = [];
  const seen = new Set();

  for (const addon of addons) {
    for (const output of getAddonBuildExpectedOutputs(addon)) {
      if (!output?.path || seen.has(output.path)) {
        continue;
      }
...
function createVisualStudioProjectBuildCommand(state, projectName) {
  const projectPath = getVisualStudioProjectPath(state, projectName);
  const msbuildPath = getVisualStudioMsBuildPath(state);
  if (!projectPath || !msbuildPath) {
    return '';
  }

  return [
    `& ${quoteShellPath(msbuildPath)}`,
    quoteShellPath(projectPath),
    `/p:Configuration=Release`,
    `/p:Platform=${getVisualStudioBuildPlatform(state)}`,
    '/m',
  ].join(' ');
}

function resolveAddonVisualStudioProjectName(state, addon) {
  const declaredTarget = addon.build?.target;
  if (declaredTarget && hasVisualStudioProject(state, declaredTarget)) {
    return declaredTarget;
  }

  if (hasVisualStudioProject(state, 'skymp5-addons')) {
    return 'skymp5-addons';
  }
...
function resolveAddonCmakeBuildTarget(state, addon) {
  ...
  if (hasVisualStudioProject(state, declaredTarget)) {
    return declaredTarget;
  }

  if (isVisualStudioGenerator(state) && hasVisualStudioProject(state, 'skymp5-addons')) {
    return 'skymp5-addons';
  }
...
function commandUsesNodePackageManager(command) {
  return /^(npm|yarn|pnpm|bun)(?:\s|$)/i.test(trimText(command));
}

function addonUsesNodePackageManager(addon) {
  if (addon.build?.kind !== 'command') {
    return false;
  }

  return (
    commandUsesNodePackageManager(addon.build.command) ||
    commandUsesNodePackageManager(addon.build.installCommand)
  );
}

function addonNeedsInstalledNodeModules(addon) {
  if (addon.build?.kind !== 'command') {
    return false;
  }

  return commandUsesNodePackageManager(addon.build.command) && !trimText(addon.build.installCommand);
}

function getAddonPackageJsonPath(addon) {
  if (addon.build?.kind !== 'command') {
    return '';
  }

  return path.join(addon.build.workingDirectory, 'package.json');
}

function createAddonBuildRequirementsForAddon(addon) {
  if (addon.build?.kind !== 'command') {
    return [];
  }

  const addonKey = sanitizeAddonKey(addon.addonId);
  const requirements = [
    createRequirement(
      `addon-${addonKey}-working-directory`,
      `${addon.label} working directory`,
      fs.existsSync(addon.build.workingDirectory),
      `Addon ${addon.addonId} build working directory is missing: ${addon.build.workingDirectory}`,
    ),
  ];

  if (addonUsesNodePackageManager(addon)) {
    requirements.push(
      createRequirement(
        `addon-${addonKey}-package-json`,
        `${addon.label} package.json`,
        fs.existsSync(getAddonPackageJsonPath(addon)),
        `Expected package.json in ${addon.build.workingDirectory} before running Node package-manager commands for addon ${addon.addonId}.`,
      ),
    );
  }

  if (addonNeedsInstalledNodeModules(addon)) {
    requirements.push(
      createRequirement(
        `addon-${addonKey}-node-modules`,
        `${addon.label} installed dependencies`,
        hasNodeModules(addon.build.workingDirectory),
        `Install dependencies in ${addon.build.workingDirectory} first (npm install or declare build.installCommand in skymp-addon.json).`,
      ),
    );
  }
...
function createAddonBuildRequirements(addons) {
  return addons.flatMap((addon) => createAddonBuildRequirementsForAddon(addon));
}
...
function buildAllRequirements(state) {
  return [
    ...
    ...createAddonBuildRequirements(
      getCommandAddons(state).filter((addon) => isAddonIncludedInBuildAll(addon)),
    ),
  ];
}
...
function buildAllDependencies(state) {
  return [
    ...
    'cmake-default',
    'front-in-tree-build',
    ...(getCmakeTargetAddons(state).length > 0 ? ['cmake-addons'] : []),
    ...getCommandAddons(state)
      .filter((addon) => isAddonIncludedInBuildAll(addon))
      .map((addon) => addonProfileId(addon)),
  ];
}
...
function buildAllSteps(state) {
  ...
  for (const addon of getCommandAddons(state).filter((candidate) => isAddonIncludedInBuildAll(candidate))) {
    steps.push(...createAddonBuildSteps(addon, state));
  }
...
function buildAllExpectedOutputs(state) {
  return [
    ...
    ...collectAddonExpectedOutputs(
      getBuildableAddons(state).filter((addon) => isAddonIncludedInBuildAll(addon)),
    ),
  ];
}
...
function createAddonBuildSteps(addon, state) {
  if (addon.build?.kind === 'cmake-target') {
    if (isVisualStudioGenerator(state)) {
      const projectName = resolveAddonVisualStudioProjectName(state, addon);
      const usesAggregateProject =
        projectName === 'skymp5-addons' && projectName !== addon.build.target;
      const projectCommand = createVisualStudioProjectBuildCommand(state, projectName);
      if (projectCommand) {
        return [
          {
            id: `addon-build-${sanitizeAddonKey(addon.addonId)}`,
            label: usesAggregateProject
              ? `Build ${addon.label} addon target via skymp5-addons`
              : `Build ${addon.label} addon target`,
            cwd: state.buildDir,
    ...
    const buildTarget = resolveAddonCmakeBuildTarget(state, addon);
    const usesAggregateTarget = buildTarget === 'skymp5-addons' && buildTarget !== addon.build.target;
    return [
      {
        id: `addon-build-${sanitizeAddonKey(addon.addonId)}`,
        label: usesAggregateTarget
          ? `Build ${addon.label} addon target via skymp5-addons`
          : `Build ${addon.label} addon target`,
        cwd: state.buildDir,
        command: cmakeBuildCommand(state, buildTarget),
      },
    ];
  }
  ...
  if (addon.build?.kind === 'command') {
    ...
    if (trimText(addon.build.installCommand)) {
      steps.push({
        id: `addon-install-${sanitizeAddonKey(addon.addonId)}`,
        label: `Install ${addon.label} addon dependencies`,
        cwd: addon.build.workingDirectory,
        command: addon.build.installCommand,
      });
    }

    steps.push({
      id: `addon-build-${sanitizeAddonKey(addon.addonId)}`,
      label: `Run ${addon.label} addon build`,
      cwd: addon.build.workingDirectory,
      command: addon.build.command,
    });
...
function createAddonProfileDefinitions(state) {
  return getBuildableAddons(state).map((addon) => ({
    id: addonProfileId(addon),
    label: `Addon: ${addon.label}`,
    description:
      addon.build?.kind === 'cmake-target'
        ? `Build the ${addon.label} addon through its declared CMake target.`
        : `Run the declared build command for the ${addon.label} addon.`,
    category: 'addons',
    dependencies: addon.build?.kind === 'cmake-target' ? ['configure-cmake'] : [],
    requirements: (profileState) => {
      const requirements = [];
      ...
      if (addon.build?.kind === 'cmake-target') {
        requirements.push(
          ...
          createRequirement(
            ...
            profileState.cmakeConfigured,
            'Run the configure-cmake profile first so the build directory contains CMakeCache.txt.',
          ),
          createCmakeOptionRequirement(profileState, 'BUILD_CLIENT', {
            id: `addon-${sanitizeAddonKey(addon.addonId)}-build-client-on`,
            enabledDetail: `This addon requires CMake to be configured with BUILD_CLIENT=ON.`,
          }),
        );

        if (isVisualStudioGenerator(profileState)) {
          requirements.push(
            createRequirement(
              `addon-${sanitizeAddonKey(addon.addonId)}-msbuild`,
      ...
      requirements.push(...createAddonBuildRequirementsForAddon(addon));

      return requirements;
    },
    steps: (profileState) => createAddonBuildSteps(addon, profileState),
    expectedOutputs: () => collectAddonExpectedOutputs([addon]),
    cmakeOptions: addon.build?.kind === 'cmake-target' ? ['BUILD_CLIENT'] : [],
  }));
}
...
function finalizeProfile(base, state) {
...
const STATIC_PROFILE_DEFINITIONS = [
  {
    id: 'configure-cmake',
  ...
  },
  {
    ...
    description:
      'Configure CMake, build the default solution, then refresh the local front output plus any discovered addon builds into the configured build directory.',
    category: 'workflow',
  ...
  },
  {
    ...
    expectedOutputs: (state) => [
      ...
      ...collectAddonExpectedOutputs(getCmakeTargetAddons(state)),
    ],
  ...
  {
    id: 'cmake-addons',
    label: 'CMake addon build',
    description: 'Build the aggregate skymp5-addons target from discovered addon manifests.',
    category: 'addons',
    dependencies: ['configure-cmake'],
    requirements: (state) => [
      createRequirement(
      ...
      createCmakeOptionRequirement(state, 'BUILD_CLIENT', {
        id: 'cmake-addons-build-client-on',
        enabledDetail: 'Addon builds require CMake to be configured with BUILD_CLIENT=ON.',
      }),
      createRequirement(
        'addon-cmake-targets',
        'Detected addon manifests',
        getCmakeTargetAddons(state).length > 0,
        `No addon manifests with build.kind="cmake-target" were found under ${path.join(state.repoRoot, 'skymp5-addons')}.`,
      ),
      createRequirement(
        'addon-msbuild',
        'Visual Studio MSBuild',
    ...
    steps: (state) => {
      const visualStudioCommand = createVisualStudioProjectBuildCommand(state, 'skymp5-addons');
      return [
        {
          id: 'build-addons',
          label: 'Build skymp5-addons target',
          cwd: state.buildDir,
          command: visualStudioCommand || cmakeBuildCommand(state, 'skymp5-addons'),
        },
      ];
    },
    expectedOutputs: (state) => collectAddonExpectedOutputs(getCmakeTargetAddons(state)),
    cmakeOptions: ['BUILD_CLIENT'],
  },
  {
...
function getProfileDefinitions(state) {
  return [...STATIC_PROFILE_DEFINITIONS, ...createAddonProfileDefinitions(state)];
}

export function listBuildProfiles(runtime) {
  const state = buildState(runtime);
  return getProfileDefinitions(state).map((definition) => finalizeProfile(definition, state));
}
