...
The dashboard also includes a `Mods` tab that inventories the current Skyrim `Data` directory, shows the resolved `Plugins.txt` state, lets you enable or disable plugin-backed mods, and compares the enabled plugin order against `build/dist/server/server-settings.json` so server startup uses the same load order.
...
| `skymp-dev watch` | Debounced re-sync when manifest-declared artifact sources change |
...
- Addon manifest schema: [`schema/addon-manifest.schema.json`](schema/addon-manifest.schema.json)
...
Addon-owned build/sync metadata is discovered automatically from `skymp5-addons/*/skymp-addon.json`. Those manifests can contribute sync artifacts, warnings, stale-removal rules, and addon-specific build profiles without editing the assistant's main YAML manifest. Command-style addon builds can also declare `build.installCommand` for dependency bootstrap such as `npm install`.
...
- Addons: `cmake-addons` plus discovered `addon-<addonId>` profiles from `skymp5-addons/*/skymp-addon.json`
- In-tree package builds: `front-in-tree-build`
...
- Compare the enabled `Plugins.txt` order against `build/dist/server/server-settings.json`
- Sync the server `loadOrder` from the Mods tab when the server settings drift out of date
