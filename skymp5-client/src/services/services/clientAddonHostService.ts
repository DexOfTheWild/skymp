...
import { registerClientAddonConsoleCommand } from "./clientAddonConsoleCommandRegistry";
...
export class ClientAddonHostService extends ClientListener {
  ...
  public hasAuthProvider(): boolean {
    return this.runtime.hasAuthProvider();
  }
  ...
  private createBrowserApi(): ClientAddonBrowserApi {
    const browser: ClientAddonBrowserApi = {
      ...
      setMediaPermissionPolicy: (
        policy: ClientAddonBrowserMediaPermissionPolicy,
      ) => {
        const browserApi = this.sp.browser as BrowserApiWithEmitEvent;
  ...
  private readonly runtime = new ClientAddonHostRuntime({
    ...
    registerConsoleCommand: (
      ...
      if (!registration.ok) {
        logError(
          this,
          `Client addon '${addonId}' failed to register console command '${commandName}'`,
          registration.error,
        );
        return () => undefined;
      }

      logTrace(this, "Registered client addon console command", {
        addonId,
