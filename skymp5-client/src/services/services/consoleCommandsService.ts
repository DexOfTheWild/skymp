...
import { getClientAddonConsoleCommand } from "./clientAddonConsoleCommandRegistry";
...
export class ConsoleCommandsService extends ClientListener {
    ...
    private getCommandExecutor(commandName: CmdName): (...args: unknown[]) => boolean {
        return (...args: unknown[]) => {
            ...
            if (commandName === "mp" && this.tryHandleAddonMpCommand(normalizedArgs)) {
                return false;
            }
    ...
    private tryHandleAddonMpCommand(args: unknown[]): boolean {
        const tokens = args
            .map((arg) => `${arg}`.trim())
        ...
        const registration = getClientAddonConsoleCommand(commandName);
        if (!registration) {
            return false;
        }
        ...
        try {
            ...
            logError(
                this,
                `Client addon console command '${commandName}' failed for addon '${registration.addonId}'`,
                error,
            );
