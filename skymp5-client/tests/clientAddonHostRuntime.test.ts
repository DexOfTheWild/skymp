...
import { getClientAddonConsoleCommand } from "../src/services/services/clientAddonConsoleCommandRegistry";
  ...
  return {
    ...
    runtime: new ClientAddonHostRuntime({
      ...
      registerConsoleCommand: (
        ...
        const { registerClientAddonConsoleCommand } = require("../src/services/services/clientAddonConsoleCommandRegistry");
        const registration = registerClientAddonConsoleCommand({
          addonId,
          commandName,
          handler,
        });
...
const testConsoleCommandRegistrationAndUnregistration = () => {
  ...
  let unregister: ClientAddonUnsubscribe = () => {
    throw new Error("Expected unregister handler");
  };
  const commandName = "addonhosttest";

  harness.runtime.registerAddon("console", (api) => {
    unregister = api.registerConsoleCommand(commandName, () => true);
  });

  assert.ok(getClientAddonConsoleCommand(commandName));
  unregister();
  assert.equal(getClientAddonConsoleCommand(commandName), undefined);
};
...
const testAuthProviderPresenceIsTracked = () => {
  const harness = createHarness({});

  assert.equal(harness.runtime.hasAuthProvider(), false);

  harness.runtime.registerAddon("auth-provider", (api) => {
    api.auth.onAuthNeeded(() => undefined);
  });

  assert.equal(harness.runtime.hasAuthProvider(), true);
};
...
testAuthProviderPresenceIsTracked();
