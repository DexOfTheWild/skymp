...
import {
  ClientAddonApi,
  ClientAddonBrowserMediaPermissionPolicy,
...
class VoipClientAddon {
  private static readonly VOICE_MODE_CYCLE_KEY = "X";
  private static readonly SECURE_ORIGIN_AUDIO_CAPTURE =
    "secureOriginAudioCapture" as ClientAddonBrowserMediaPermissionPolicy;
  ...
  private onLocalSpawn(): void {
    ...
    if (
      this.api.capabilities.browser.mediaPermissionPolicies.includes(
        VoipClientAddon.SECURE_ORIGIN_AUDIO_CAPTURE,
      )
    ) {
      this.api.browser.setMediaPermissionPolicy(VoipClientAddon.SECURE_ORIGIN_AUDIO_CAPTURE);
    } else {
      this.api.log(
  ...
  private onInputState(inputState: ClientAddonInputState): void {
    ...
    if (cycleKeyDown && !this.wasVoiceModeCycleKeyDown) {
      ...
      this.api.log("VoIP voice mode changed", {
        key: VoipClientAddon.VOICE_MODE_CYCLE_KEY,
  ...
  private getVoiceModeCycleScanCode(): number {
    const scanCode = this.api.resolveScanCode(VoipClientAddon.VOICE_MODE_CYCLE_KEY);
    if (scanCode !== null) {
      return scanCode;
    }

    this.api.logError(
      `VoIP voice mode cycle key '${VoipClientAddon.VOICE_MODE_CYCLE_KEY}' is unavailable in the host runtime`,
    );
    return -1;
  }
...
export const registerVoipClientAddon = (api: ClientAddonApi): void => {
  new VoipClientAddon(api);
};
