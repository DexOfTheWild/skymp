...
class VoipServerAddon implements ServerAddon {
  public constructor(
    private api: ServerAddonApi,
    config: VoipServerAddonConfig,
  ) {
  ...
  public systemName = "VoipServerAddon";
  ...
  public async update(): Promise<void> {
    ...
    for (const [userId, profileId] of this.profileIdByUserId.entries()) {
      ...
      this.api.sendCustomPacket(
        userId,
        VOIP_POLICY_STATE_PACKET_TYPE,
        createVoipPolicyStatePacket(nextState),
      );
    }
  }
...
export const addonId = "voip";

export const createServerAddon: ServerAddonModule["createServerAddon"] = (
  api,
  config,
) => {
  return new VoipServerAddon(api, (config || {}) as VoipServerAddonConfig);
};
