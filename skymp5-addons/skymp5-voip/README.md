...
`npm run build` regenerates that file into
`skymp5-addons/build/dist/client/Data/Platform/Plugins/skymp5-voip-settings.txt`.
By default it derives the URLs from `VOIP_PUBLIC_HOST` and `VOIP_UI_PORT`, and
you can override the generated client settings with:

- `VOIP_CLIENT_ENABLED`
- `VOIP_CLIENT_POSITIONAL_AUDIO_MODE`
- `VOIP_CLIENT_PTT_KEY`
- `VOIP_CLIENT_UI_URL`
- `VOIP_CLIENT_RAW_UI_URL`
...
- client plugin bundle:
  - `skymp5-addons/build/dist/client/Data/Platform/Plugins/skymp5-voip.js`
- generated client settings:
  - `skymp5-addons/build/dist/client/Data/Platform/Plugins/skymp5-voip-settings.txt`
- server plugin bundle:
  - `skymp5-addons/build/dist/voip/server/skymp5-voip-server-plugin.js`
- voip dev service bundle:
  - `skymp5-addons/build/dist/voip/voip-dev-server.js`
- plugin-owned browser UI:
  - `skymp5-addons/build/dist/voip/ui/*`

## Basic Build / Run Flow

```powershell
cd skymp5-addons/skymp5-voip
npm install
npm run build
npm run start
```
...
- client host version `6` exposes `onLocalSpawn`, `onTick`,
  `getLocalProfileId`, `getSettingsScope`, `resolveScanCode`,
  `browser.getBackendName()`, `browser.setMediaPermissionPolicy()`, and
  `capabilities.browser.mediaPermissionPolicies`
