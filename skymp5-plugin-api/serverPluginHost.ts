...
export type ServerPluginCustomPacketHandler = (
  userId: number,
  content: ServerPluginContent,
) => void | Promise<void>;
export type ServerPluginCapabilities = {
  actorAngleZ: boolean;
...
export type ServerPluginApi = {
  capabilities: ServerPluginCapabilities;
  ...
  onCustomPacket: (
    type: string,
    handler: ServerPluginCustomPacketHandler,
  ) => (() => void);
  onSpawnAllowed: (
