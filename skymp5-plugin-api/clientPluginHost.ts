...
export const SKYMP_CLIENT_PLUGIN_HOST_VERSION = 4 as const;

export type ClientPluginBrowserMediaPermissionPolicy =
  | "default"
  | "secureOriginAudioCapture";

export type ClientPluginCapabilities = {
  browser: {
    mediaPermissionPolicies: ClientPluginBrowserMediaPermissionPolicy[];
  };
...
export type ClientPluginConsoleCommandContext = {
  commandName: string;
  pluginId: string;
...
export type ClientPluginConsoleCommandHandler = (
  args: string[],
  context: ClientPluginConsoleCommandContext,
) => boolean | void;
