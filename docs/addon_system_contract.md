...
- Current API version: client `6`, server `3`
...
- Media-permission policy control is advertised through
  `capabilities.browser.mediaPermissionPolicies`. Addons should check
  capabilities instead of duck-typing browser methods.
- Console command support is advertised through
  `capabilities.consoleCommands`.
- Addon-defined local console commands currently register as `mp`
